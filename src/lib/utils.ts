import forge from 'node-forge';

const PEM_PUBLIC_KEY = `-----BEGIN PUBLIC KEY-----

owIDAQAB
-----END PUBLIC KEY-----`;


export async function encryptWithRsa(plainText: string): Promise<string> {
  if (!plainText) {
    throw new Error('加密明文不能为空');
  }

  try {
    // 1. 解析 PEM 公钥
    const publicKey = forge.pki.publicKeyFromPem(PEM_PUBLIC_KEY);

    // 2. 执行 RSA-OAEP 加密（默认 SHA-1，需显式指定 SHA-256）
    const encrypted = publicKey.encrypt(
      plainText,
      'RSA-OAEP',
      {
        md: forge.md.sha256.create(), // 👈 关键：指定 SHA-256
        mgf1: {
          md: forge.md.sha256.create()
        }
      }
    );

    // 3. 返回 Base64（forge 输出是二进制字符串）
    return forge.util.encode64(encrypted);
  } catch (err) {
    throw new Error(`加密失败: ${err.message}`);
  }
}


// export async function encryptWithRsa(plainText: string): Promise<string> {
//   if (!plainText) {
//     throw new Error('加密明文不能为空');
//   }

//   // 2. 清理公钥字串，提取纯 Base64 部分
//   const b64Clean = PEM_PUBLIC_KEY
//     .replace(/-----\s*BEGIN[^-]*-----\s*/g, '')
//     .replace(/-----\s*END[^-]*-----\s*/g, '')
//     .replace(/\s/g, '');

//   // 3. 转换为二进制 Uint8Array
//   const binaryStr = atob(b64Clean);
//   const bytes = new Uint8Array(binaryStr.length);
//   for (let i = 0; i < binaryStr.length; i++) {
//     bytes[i] = binaryStr.charCodeAt(i);
//   }

//   // 4. 浏览器原生导入公钥
//   const cryptoKey = await window.crypto.subtle.importKey(
//     'spki',
//     bytes.buffer,
//     { 
//       name: 'RSA-OAEP', 
//       hash: 'SHA-256' 
//     },
//     false,
//     ['encrypt']
//   );

//   // 5. 执行加密
//   const textBytes = new TextEncoder().encode(plainText);
//   const encryptedBuffer = await window.crypto.subtle.encrypt(
//     { name: 'RSA-OAEP' },
//     cryptoKey,
//     textBytes
//   );
  
//   // 6. 返回 Base64 密文
//   const encryptedBytes = new Uint8Array(encryptedBuffer);
//   return btoa(String.fromCharCode(...encryptedBytes));
// }
